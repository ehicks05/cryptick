...
const toMinimumDigits = (increment: string) => {
	const withoutTrailingZeroes = increment.substring(0, increment.indexOf('1') + 1);
	const withoutLeadingZero = withoutTrailingZeroes.substring(
		withoutTrailingZeroes.indexOf('.') + 1,
	);
	return withoutLeadingZero.length;
};
...
const _exchangeInfo = async () => {
	const response = await client({ path: '/exchangeInfo' });
